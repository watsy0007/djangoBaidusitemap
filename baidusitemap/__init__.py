    ...
    def get_urls(self, page=1, site=None, protocol=None):
        ...
        for item in self.paginator.page(page).object_list:
            ...
            if hasattr(self, 'wml_url'):
                url_info['wml_url'] = "%s://%s%s" % (protocol, domain, self.__get('wml_url', item))
            if hasattr(self, 'xhtml_url'):
                url_info['xhtml_url'] = "%s://%s%s" % (protocol, domain, self.__get('xhtml_url', item))
            if hasattr(self, 'html_url'):
                url_info['html5_url'] = "%s://%s%s" % (protocol, domain, self.__get('xhtml_url', item))
